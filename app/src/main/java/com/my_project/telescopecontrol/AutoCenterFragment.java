...
public class AutoCenterFragment extends Fragment implements CameraBridgeViewBase.CvCameraViewListener2 {
        ...
        viewModel.get_dec_returned_current_position().observe(getViewLifecycleOwner(), new Observer<Integer>() {
            ...
            public void onChanged(Integer received_position) {
                ...
                if (!calibration_started && calibration_finished) {
                    ...
                    distance_moved_during_calibration = abs(sqrt((brightest_point_before_testing.x - brightest_point.x) *
                            (brightest_point_before_testing.x - brightest_point.x) + (brightest_point_before_testing.y - brightest_point.y) *
    ...
    @Override
        ...
        if (contours.size() > 0) {
            ...
            Imgproc.drawContours(mRgba, contours, max_contour_area_index, green, 2);
            max_contour_area_2f.fromArray(contours.get(max_contour_area_index).toArray());
        ...
        if (calibration_started) {
            current_rotation = rotation(brightest_point, brightest_point_before_testing);
            // 90 degrees added because the frame in preview is rotated by 90°
    ...
    private void start_moving_dec_motor() {
        ...
        if (viewModel.get_dec_goto_ending_direction().getValue() == -1) {
            start_testing_dec = true;
            //save distance before the move starts so later we can get the new distance and compare with the saved one :
            if (distance_from_center != 0) {
                distance_before_dec_test = distance_from_center;
                brightest_point_before_testing.x = brightest_point.x;
                brightest_point_before_testing.y = brightest_point.y;
